...
def login_provider(provider):
    """This is only for Facebook and Google login/signup"""
    ...
    if request.method == 'POST':
        ...
        # Validate if the auth exists
        user = AuthMethod.user_method_get_user_by_email(data['email'])

        if not user:
            print('User does not exist... creating new user...')
            print('session: ', session)
            user_id = AuthMethod.user_method_create_user(authenticated=session['authenticated'], email=session['email'],
                                                         first_name=session['first_name'],
                                                         last_name=session['last_name'],
                                                         password=session['password'], picture=session['picture'],
                                                         profile=session['profile'], provider=session['provider'],
                                                         session_token=session['session_token'],
                                                         username=session['username'])
            print('user_id: ', user_id)
            user = AuthMethod.user_method_get_user_by_id(user_id)
            print('user: ', user)
...
def logout():
    ...
    if 'username' in session:
        ...
        user_id = AuthMethod.user_method_get_id_by_session_token(session['session_token'])
        ...
        auth = AuthMethod.get_first_link_by_user_id(user_id=user_id)
        # Auth.query.filter_by(user_id=user_id).order_by(Auth.login_time.desc()).first()
